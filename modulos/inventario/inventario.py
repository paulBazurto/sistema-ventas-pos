...
from tkinter import ttk, messagebox, filedialog, LabelFrame   # <--- Importar LabelFrame
...
class Inventario(tk.Frame):
    ...
    def widgets(self):
        #---------------------------------------------------------------------------------
        canvas_articulos = LabelFrame(self, text="Articulos", font='arial 15 bold', bg='#C6D9E3')
    ...
    def articulos_combobox(self):
        """Cargar lista de artículos (versión MySQL)"""
        conn = get_connection()
        if not conn:
            self.articulos = []
    ...
    def editar_articulo(self):
        ...
        tk.Button(top, text='Guardar', font="arial 12 bold", command=guardar).place(x=50, y=260, width=150, height=40)
        tk.Button(top, text='Cancelar', font="arial 12 bold", command=top.destroy).place(x=260, y=260, width=150, height=40)
        tk.Button(top, bg="red", fg="white", text='Eliminar artículo', font="arial 12 bold", command=eliminar).place(x=260, y=330, width=150, height=40)